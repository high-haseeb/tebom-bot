...
export const startOfferProcess = async (data: TrafficInfo) => {
    ...
    try {
        const response = await fetch("http://localhost:4040/startOffers", {
            method: "POST",
...
export const getTrafficInformation = async () => {
    const url = "http://localhost:4040/get/vehicleInfo";
    const requestData = {
...
export async function getListOffers() {
    const response = await fetch("http://localhost:4040/get/offers");
